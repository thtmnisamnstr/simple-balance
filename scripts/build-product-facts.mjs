...
// No `$schema`. The file used to name one at smpl.money, and nothing at that
// address has ever been served by either repository, so an editor or a
// validator following it met the site's 404 page. A schema is worth naming
// only once somebody publishes it, and the contract's shape is held by
// `tests/product-facts.test.ts` meanwhile.
const facts = {
  $comment:
