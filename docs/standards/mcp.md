...
- **House, and the half a count cannot see: a shared description has to be true
  ...
  (src/shared/domain.ts:1876-1880`), and the other four derive from it —
  `bulkTransactionFilterSchema` by `.omit()` at `:1897` and
  `stageListQuerySchema` at `:2171`, which is a sixth position nobody had
...
- **House.** The code list is closed and published. `serviceErrorCodes`
  (src/shared/domain.ts:2454-2464`) is a `const` array rather than a bare
