...
- **House, and a live gap.** Unknown query parameters and unknown body fields
  ...
  `listQuerySchema` (src/shared/domain.ts:1599-1668`) accepts anything, so
  ...
  **is** strict (src/shared/domain.ts:1673-1675`), so the two disagree about
...
- **House, settled.** A boolean query parameter is read one way.
  ...
  **on**, so `?includeArchived=1` turned them off without saying so, and on that
  ...
  one (src/shared/domain.ts:1317-1338`) and the route hands the schema the raw
...
plan, absent leaves it alone (src/shared/domain.ts:1470-1494`). So does the
...
and `notes` (src/shared/domain.ts:1831-1842`), pinned by
`tests/domain.test.ts:142-149`, and the staged bulk patch carries the identical
transform on the same two fields (src/shared/domain.ts:1788-1799`). The
...
- **House.** Two list envelopes:
  - `Page<T>`: `{items, nextCursor}` (src/shared/domain.ts:2278-2282`), where
    callers only stream forward.
  - `PaginatedPage<T>`: `Page<T>` plus `{page, pageSize, totalCount,
    totalPages}` (src/shared/domain.ts:2284-2289`).
...
- **House.** The published enumeration is frozen contract, and it is complete.
  `apiErrorCodes` (src/shared/domain.ts:2233-2268`) is the sum of two lists
...
- **House.** `limit` is optional, defaults to 50 and is capped at 200
  (src/shared/domain.ts:1298`). A server may return fewer rows than asked for.
...
- **House.** Sorting is `sort` plus `direction`, with `direction` one of `asc`
  or `desc` (src/shared/domain.ts:1602-1613`). If multi-key sorting ever
...
- **Binding.** Order is presentation and never scopes a write. `sort`,
  ...
  schema (src/shared/domain.ts:1673-1675`), so two requests selecting the same
...
- **House, one named exception.** `PUT /api/v1/budget-entries` is an upsert and
  ...
  (src/shared/domain.ts:1497-1505`).
...
- **House, and violated on five creates.** `AGENTS.md` says creates that write
  ...
  `POST /categories/merge` do not (`src/shared/domain.ts:958`, `:992`, `:2849`,
  `:2811`, `:1007`). The MCP tools for the same operations all require one.
...
- **House, scoped to what the invariant above covers: transaction and staged
  ...
  (src/shared/domain.ts:1759-1762`):
