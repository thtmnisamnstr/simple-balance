...
as an id that was never issued (`src/server/services/accounts.ts:533`). This is
...
both of the others (src/server/mcp.ts:284-289`). Routes marked session only are
named exceptions in `tests/mcp-parity.test.ts:19-28`, each carrying its reason.
...
stolen from (src/server/api.ts:1094-1098`).
...
| Route | What it is |
| --- | --- |
| `GET /health/live`, `GET /health/ready` | Liveness, and a `select 1` against the database. `503` when the database is unreachable (src/server/api.ts:299-314`). |
| `/api/auth/*` | Better Auth, plus this product's own sign-up, consent and MCP token routes. |
| `/.well-known/oauth-protected-resource`, `/.well-known/oauth-authorization-server`, `/.well-known/openid-configuration` | RFC 9728 and OAuth discovery, each also served under `/mcp` and `/mcp/` because RFC 9728 puts the resource path after the well-known segment (src/server/api.ts:860-868`). |
...
*Checked by:* tests/mcp-parity.test.ts:107-114` extracts the registered
...
- **House.** Every path id is a UUID and is parsed at the boundary before it
  reaches a query, through `pathId` (src/server/api.ts:1003-1005`). Nothing in a
...
- **House, and the three inconsistencies that were here are gone.** The paths
  ...
  `GET /api/v1/staged-transactions/{id}/duplicate` (src/server/api.ts:1434`)
  rather than a `staged` collection that existed nowhere else;
  `POST /api/v1/staged-transactions/bulk-delete` (src/server/api.ts:1397`)
  rather than a `delete` that spelled the same operation as
  `POST /api/v1/transactions/bulk-delete` (src/server/api.ts:1337`)
  differently; and `POST /api/v1/accounts/{id}/archived` and
  `POST /api/v1/categories/{id}/archived` (src/server/api.ts:1178`, `:1315`),
...
- **House.** A malformed or absent JSON body is a 400 with a message saying so,
  ...
  (src/server/api.ts:985-995`); before it existed a truncated body arrived as a
...
- **House.** Request bodies are bounded, and the bound is derived from a
  ...
  (`src/server/http-security.ts:60-98`, `:639-662`). A limit is derived, not
...
- **House, and a live gap.** Unknown query parameters and unknown body fields
  ...
  `listQuerySchema` (src/shared/domain.ts:1769-1838`) accepts anything, so
  ...
  **is** strict (src/shared/domain.ts:1843-1845`), so the two disagree about
...
- **House, settled.** A boolean query parameter is read one way.
  ...
  `includeArchivedFlag` (src/server/api.ts:1022`), which parses with the shared
  schema.
  ...
  **on**, so `?includeArchived=1` turned them off without saying so, and on that
  ...
  one (src/shared/domain.ts:1392-1413`) and the route hands the schema the raw
...
plan, absent leaves it alone (src/shared/domain.ts:1635-1663`). So does the
...
and `notes` (src/shared/domain.ts:2001-2012`), pinned by
`tests/domain.test.ts:142-149`, and the staged bulk patch carries the identical
transform on the same two fields (src/shared/domain.ts:1958-1969`). The
...
- **House.** Two list envelopes:
  - `Page<T>`: `{items, nextCursor}` (src/shared/domain.ts:2448-2452`), where
    callers only stream forward.
  - `PaginatedPage<T>`: `Page<T>` plus `{page, pageSize, totalCount,
    totalPages}` (src/shared/domain.ts:2454-2459`).
...
- **House.** `GET /api/v1/csv/export` is the only route that answers with
  ...
  `Content-Disposition` filename (src/server/api.ts:1415-1429`). Its format is
...
| 429 | Rate limited. Today only the setup-code limiter (src/server/api.ts:390-419`) |
...
- **House, and a live inconsistency.** One code must map to one status.
  ...
  and 400 from the malformed-body guard (src/server/api.ts:993`), so on that
...
- **House, and a gap.** A missing `expectedVersion` should be `428 Precondition
  ...
  (`src/shared/domain.ts:226-236`, src/server/api.ts:278-285`).
- **House, and a mismatch.** A path id that is not a UUID can never name a row,
  ...
  (src/server/api.ts:1003-1004`) which the global handler renders as a 422
  (src/server/api.ts:275-284`), so a mistyped URL and a rejected body look the
...
- **House, and a gap.** A wrong method on an existing path should be 405 with
  ...
  (src/server/api.ts:1472-1476`). OWASP's REST guidance is to allowlist methods
...
- **House.** The published enumeration is frozen contract, and it is complete.
  `apiErrorCodes` (src/shared/domain.ts:2403-2438`) is the sum of two lists
  ...
  `UNSUPPORTED_MEDIA_TYPE` (`:180`, `:219`), `PAYLOAD_TOO_LARGE` (`:573`,
  `:618`), `INVALID_CONTENT_LENGTH` (`:562`) and `REQUEST_BODY_NOT_ALLOWED`
  (`:588`). All fourteen reach a caller from `/api/v1` in this guide's own
...
- **House, and violated in fourteen places.** One envelope on every route this
  ...
  routes return a flat `{code, message}` at src/server/api.ts:351`, `:379`,
  `:396`, `:405`, `:417`, `:436`, `:447`, `:484`, `:641`, `:720`, `:727`,
  `:736`, `:769` and `:777`. The browser can only read one of the two shapes.
  Fix those fourteen.
  **One named exception:** the `/.well-known` catch-all returns
  `{error, error_description}` (src/server/api.ts:883-891`). That is the OAuth
...
- **House.** Field errors are `{field, message}` with `field` a dotted path.
  ...
  (src/server/mcp.ts:284`), but the global HTTP handler does not: it ships
  `error.issues` straight from Zod (src/server/api.ts:281`), putting the
...
- **House.** No stack traces, no SQL, no bound parameters, ever. The comment at
  src/server/api.ts:287-293` says why, and it is not boilerplate: "Drizzle
...
- **House, and an overload to fix.** `nextCursor: null` currently means two
  ...
  (`src/server/services/transactions.ts:1407-1424`). A client cannot tell them
...
- **House.** `limit` is optional, defaults to 50 and is capped at 200
  (src/shared/domain.ts:1365`). A server may return fewer rows than asked for.
- **House, and an outlier.** `GET /api/v1/audit-events` parses its own query by
  hand rather than through a published schema
  (src/server/api.ts:1457-1464`), so its parameters are the one list contract
...
- **House.** Sorting is `sort` plus `direction`, with `direction` one of `asc`
  or `desc` (src/shared/domain.ts:1772-1783`). If multi-key sorting ever
...
- **Binding.** Order is presentation and never scopes a write. `sort`,
  ...
  schema (src/shared/domain.ts:1843-1845`), so two requests selecting the same
...
- **House, and the reason is the standing test.** The version travels in the
  ...
  (src/shared/domain.ts:1107-1122`). A mismatch is `409 STALE_VERSION` with
...
- **House, and the objection is open on HTTP today.** Over MCP every mutation is
  ...
  (`runIdempotentMcpMutation`, src/server/mcp.ts:294-317`, used on twenty-six
  ...
  version and nothing else (src/shared/domain.ts:1107-1122`). So an HTTP update
...
- **House, one named exception.** `PUT /api/v1/budget-entries` is an upsert and
  ...
  (src/shared/domain.ts:1666-1675`).
...
- **House, and violated on five creates.** `AGENTS.md` says creates that write
  ...
  (src/shared/domain.ts:1094-1098` and `:1138-1143`) and `POST /accounts`,
  `POST /categories`, `POST /recurrences`, `POST /transaction-templates` and
  `POST /categories/merge` do not (`src/shared/domain.ts:958`, `:992`, `:2946`,
  `:2981`, `:1015`). The MCP tools for the same operations all require one.
  ...
  (src/shared/domain.ts:1065-1077`), so the two merges disagree about the same
...
- **House.** No `GET` or `DELETE` accepts a key. A safe method needs none, and a
  ...
  `DELETE /api/v1/connected-apps/{clientId}` (src/server/api.ts:1097-1101`)
  reads no body and takes no `expectedVersion`, where the other six versioned
  deletes parse `versionedMutationSchema` (src/server/api.ts:1184`, `:1211`,
  `:1236`, `:1244`, `:1279`, `:1316`). Revoking a grant is idempotent anyway,
...
- **House, scoped to what the invariant above covers: transaction and staged
  ...
  (src/shared/domain.ts:1929-1932`):
...
- **House, and three routes outside that scope encode a selection their own
  ...
  `transactionTemplateBulkSelectionSchema` (`src/shared/domain.ts:773-805`) is
  ...
  (`src/server/services/staging.ts:1089-1094`), so a commit retried across the
...
- **House.** A filter selection is resolved first by the matching
  ...
  must send back (src/server/api.ts:1323-1329`, `:1386-1388`). The fingerprint
...
- **House, and all seven bulk routes carry it.** `dryRun` is the substitute for
  ...
  (src/shared/domain.ts:1214-1238`), did not, which made the one bulk write that
...
- **House, and it declines AIP-151.** No long-running operations and no job
  ...
  **The one operation that outgrows this is CSV export**, which buffers up to
  100,000 transactions in memory (`src/server/services/import-export.ts:1007`)
...
- **House.** Everything under `/api/v1` is `Cache-Control: no-store`, without
  ...
  (src/server/api.ts:965-970`). RFC 9111's shared-cache protection keys off the
...
- **House.** The security headers are one exported function of `isProduction`
  ...
  JWKS route (src/server/api.ts:623`) and on discovery (`:847`), and
  `Cache-Control` on those two (`:629`, `:848`) and on `/api/v1` itself
  (`:925`). The
...
- **Contested, and now decided: CORS.** The rule used to be that `/api/v1`
  ...
  - **Two exceptions exist today and are correct**, both outside `/api/v1`:
    `GET /api/auth/mcp/jwks` (src/server/api.ts:622`) and the OAuth discovery
    endpoints (`discoveryHeaders`, src/server/api.ts:838-846`) both send
...
- **House.** The single-page app never answers an API path. JSON 404 catch-alls
  sit under `/api/v1/*` (src/server/api.ts:1472-1476`) and `/.well-known/*`
  (src/server/api.ts:883-887`), below every route those prefixes own and above
...
- **House, a deliberate absence.** A 401 on a cookie-authenticated `/api/v1`
  ...
  (src/server/api.ts:962-984`). A Bearer challenge there would invite an agent
...
- **Binding**, RFC 9728. Protected resource metadata is served at the root and
  ...
  appended (src/server/api.ts:860-868`). Answering only at the root left the
...
- **Binding.** The scopes are `ledger:read`, `ledger:stage` and `ledger:write`,
  published in the discovery documents (src/server/api.ts:820-829`), and the
...
One middleware sets both headers (src/server/api.ts:1157-1170`), the value of
