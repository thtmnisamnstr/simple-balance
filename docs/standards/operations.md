...
hand. src/server/api.ts:281-287` narrows a Drizzle error the same way for a
...
returns 200 or 503 (src/server/api.ts:293-308`, and the same pair on the
...
against the database and nothing else, and src/server/api.ts:293-299` says the
...
`METRICS_ENABLED=true` (src/server/api.ts:238`). Registered rather
