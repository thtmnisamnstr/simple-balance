...
src/server/services/categories.ts:487`).
...
`src/server/api.ts` constructs `AppError` directly at `:994`, `:1039`, `:1048`
and `:1061`. One carries a code a constructor covers and three carry codes no
...
did not. One of them now does — the failed password update at `:1073`, which was
a 422 with a message and had no reason to spell the constructor out. The other
cannot: the malformed-body guard at `:994` is a **400**, not a 422, because a
