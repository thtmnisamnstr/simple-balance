...
| | Files | Tests |
| --- | --- | --- |
| `npm test`, no database | 142 pass, 63 skip | **1,474 pass, 706 skip** |
| `npm test`, database set | 205 pass | **2,180 pass** |
| `npm run test:integration` | 64 pass | 707 pass |

The integration tier reports 707 tests on its own and 706 skips inside a
...
1,474 rather than among the skips, which is why the two rows add up to 2,180
both times.
...
The first row is what CI and `npm run verify` see, and 1,474 is the number that
