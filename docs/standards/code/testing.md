...
| | Files | Tests |
| --- | --- | --- |
| `npm test`, no database | 142 pass, 63 skip | **1,471 pass, 705 skip** |
| `npm test`, database set | 205 pass | **2,176 pass** |
...
1,471 rather than among the skips, which is why the two rows add up to 2,176
both times.
...
The first row is what CI and `npm run verify` see, and 1,471 is the number that
