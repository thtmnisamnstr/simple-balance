...
**Where this is broken today: 13 sites.** `react/set-state-in-effect` counts
them — 8 in `forms.tsx`, 2 in `TransactionBrowser.tsx`, one each in
`CategoriesPage.tsx`, `StagingPage.tsx` and `theme.ts`. Some are genuine
synchronisation with something outside React and are fine. Some are derived
values that should not be state. They are under a budget rather than fixed,
because deciding which is which is a change per site.

*Checked by:* `tests/lint-budget.test.ts`, which refuses to let the count grow.

### 1.4 A dependency array is exhaustive or it explains itself

**House.** 17 `react-hooks/exhaustive-deps` warnings, and they are not all the
...
The second kind is worth reading as archaeology. The category picker's
`useMemo` listed `type` for a while after the filter that used `type` was
removed, because a category running against the direction became a refund and
the list stopped narrowing.

*Checked by:* `tests/lint-budget.test.ts`, at 17. The number goes down or the
test fails.
...
| 1.2 Query key shape | A test could assert every key is an array of primitives. Worth building. |
...
Four `human` rules in this guide, and 3.3 is the one that has already cost
something.
