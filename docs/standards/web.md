...
label="Category">` wraps `CategoryLegs` at src/client/forms.tsx:1116`, `:2240`
and `:2890`, which renders up to fifty rows of three inputs.
...
fail on correct code: src/client/forms.tsx:1245` and `:2953` both use it for
...
`TransactionTypeChoice` (src/client/forms.tsx:448-576`, the function itself
at `:463`) is the reference
...
own: src/client/forms.tsx:324`, `:595` and `:2130` carry `list` and nothing
...
forms.tsx:531` and the `-1` that lets the error summary take focus without
...
Six submit controls are disabled on a computed predicate: forms.tsx:2355`
...
the split remainder line at forms.tsx:774-778`. A disabled submit button always
...
6. **No `type="number"` on a field bound to a decimal-string money value.**
   Scope it, or it fails on the recurrence interval at forms.tsx:1245` and
   `:2953` and gets deleted on first contact.
