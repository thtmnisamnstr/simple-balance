...
- **A blank-cell placeholder.** An em dash on ten sites
  (`TransactionBrowser.tsx:843`, `:851`, `:855`, `AccountDetailPage.tsx:105`,
  BudgetsPage.tsx:847`, `:853`, `:858`, `:873`, `StagingPage.tsx:761`,
  `:828`), an italic muted
  word on others, and `Uncategorized` styled `.subtle` on one page and bare on
  another. One of the eight, `TransactionBrowser.tsx:851`, writes the dash as
...
label="Category">` wraps `CategoryLegs` at src/client/forms.tsx:1117`, `:2213`
and `:2863`, which renders up to fifty rows of three inputs.
...
fail on correct code: src/client/forms.tsx:1246` and `:2926` both use it for
...
`TransactionTypeChoice` (src/client/forms.tsx:449-577`, the function itself
at `:464`) is the reference
...
own: src/client/forms.tsx:325`, `:596` and `:2130` carry `list` and nothing
...
select-all checkboxes pass it: `TransactionBrowser.tsx:790`,
`TemplatesPage.tsx:471`, `StagingPage.tsx:685`.
...
forms.tsx:532` and the `-1` that lets the error summary take focus without
...
| Where | Treatment |
| --- | --- |
| `DashboardPage.tsx:124`, `:160`, `AccountsPage.tsx:250`, `ReportsPage.tsx:247`, `:254`, `:272`, `AccountDetailPage.tsx:216` | `money-negative` on the value, with Intl's own minus sign. Still the rule for a computed total, which has a sign of its own |
| `TransactionBrowser.tsx:890-895` | Coloured and signed by transaction *type*, with a hand-prefixed `+` or `−` |
| `StagingPage.tsx:825`, `TemplatesPage.tsx:550`, `RecurrencesPage.tsx:216` | No colour and no sign |
...
above formatted tables (`DashboardPage.tsx:138` and `ReportsPage.tsx:191` were
...
Six submit controls are disabled on a computed predicate: forms.tsx:2328`
...
the split remainder line at forms.tsx:775-779`. A disabled submit button always
...
(`components.tsx:229`, `TransactionBrowser.tsx:672`, `TemplatesPage.tsx:414`),
...
6. **No `type="number"` on a field bound to a decimal-string money value.**
   Scope it, or it fails on the recurrence interval at forms.tsx:1246` and
   `:2926` and gets deleted on first contact.
