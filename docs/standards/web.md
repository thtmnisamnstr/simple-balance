...
Query for server state, and 3,497 lines of hand-written CSS in
...
BudgetsPage.tsx:698`, `DashboardPage.tsx:204`) are all runtime geometry and are
...
`styles.css:731-735` for `.input` and at `styles.css:3041-3043` for
...
stylesheet in descending order: 1050px (`styles.css:3294`), 980px
(`styles.css:3318`), 780px (`styles.css:3325`) and 560px (`styles.css:3408`).
...
`visibility`, `styles.css:3329-3331`), and there are two reduced-motion
...
(section 7.3), and `styles.css:3488-3497`, a blanket rule setting
...
- **A blank-cell placeholder.** An em dash on ten sites
  ...
  BudgetsPage.tsx:847`, `:853`, `:858`, `:873`, `StagingPage.tsx:862`,
  `:1039`), an italic muted
...
label="Category">` wraps `CategoryLegs` at src/client/forms.tsx:1142`, `:2251`
and `:2901`, which renders up to fifty rows of three inputs.
...
fail on correct code: src/client/forms.tsx:1271` and `:2964` both use it for
...
`TransactionTypeChoice` (src/client/forms.tsx:489`, the function itself
at `:489`) is the reference
...
own: src/client/forms.tsx:334`, `:621` and `:2168` carry `list` and nothing
...
`TemplatesPage.tsx:471`, `StagingPage.tsx:766`.
...
forms.tsx:557` and the `-1` that lets the error summary take focus without
...
| `.nav-scrim` | `styles.css:3347` | fixed |
| `.mobile-header` | `styles.css:3357` | sticky |
...
| Where | Treatment |
| --- | --- |
| `DashboardPage.tsx:124`, `:160`, `AccountsPage.tsx:250`, `ReportsPage.tsx:324`, `:331`, `:349`, `AccountDetailPage.tsx:216` | `money-negative` on the value, with Intl's own minus sign. Still the rule for a computed total, which has a sign of its own |
| `TransactionBrowser.tsx:892-897` | Coloured and signed by transaction *type*, with a hand-prefixed `+` or `−` |
| `StagingPage.tsx:1036`, `TemplatesPage.tsx:550`, `RecurrencesPage.tsx:216` | No colour and no sign |
...
above formatted tables (`DashboardPage.tsx:138` and `ReportsPage.tsx:244` were
...
each other or the graduated lines". `.chart-grid` at `styles.css:3030-3038` is
...
This product keeps ten, on measured grounds recorded at `styles.css:3082-3108`.
...
it is not.** `BarChart` at `src/client/charts.tsx:423-430` lays each series' bar
at `index * barWidth` with no gap (`charts.tsx:396`, `:427`), so bars within a group
touch, and `.chart-bar` sets `stroke: none` (`styles.css:3055-3057`). Two
...
round for the reader who needs it. `ChartLegend` (`src/client/charts.tsx:445-455`)
...
table structure entirely. `ReportsPage.tsx:250-268` already does this correctly,
...
Six submit controls are disabled on a computed predicate: forms.tsx:2366`
(`!splitSettled`), `TemplatesPage.tsx:682` (`!anyChange`), `SettingsPage.tsx:475`
(`!matches`), `PayeesPage.tsx:189` (`!selectedTarget`), CategoriesPage.tsx:105`
(`!trimmed`) and CategoriesPage.tsx:403` (`!target || sourceCategories.length
=== 0`). Only the first sits beside a sentence saying which condition is unmet,
the split remainder line at forms.tsx:800-804`. A disabled submit button always
...
(`components.tsx:229`, `TransactionBrowser.tsx:674`, `TemplatesPage.tsx:414`),
...
6. **No `type="number"` on a field bound to a decimal-string money value.**
   Scope it, or it fails on the recurrence interval at forms.tsx:1271` and
   `:2964` and gets deleted on first contact.
