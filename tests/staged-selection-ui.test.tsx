...
import type { Account, Page, StagedTransaction } from "../src/client/api.js";
...
/** Serves a two-page staged queue and records how often page two was fetched. */
...
const selectAllMatching = () =>
  screen.queryByRole("button", {
    name: "Select all matching staged transactions",
  });
...
describe("staged transaction selection", () => {
  ...
  it("selects only the rows on the page without fetching the rest", async () => {
    ...
    expect(stageCursors.filter((cursor) => cursor === "page-two")).toHaveLength(
      0,
    );
  });

  it("offers whole-list selection only once the page is fully selected", async () => {
    stubStagedQueue();
    renderStaging();
    expect(await screen.findByText("Visible one")).toBeInTheDocument();
  ...
  it("extends the selection across every remaining page", async () => {
    ...
    // Nothing is left to reach, so the escalation is withdrawn.
    expect(selectAllMatching()).not.toBeInTheDocument();
  });
